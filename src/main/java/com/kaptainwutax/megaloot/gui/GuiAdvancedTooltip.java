package com.kaptainwutax.megaloot.gui;
...
public class GuiAdvancedTooltip extends GuiButton {
	...
	public GuiAdvancedTooltip(int buttonId, int icon, int x, int y, int width, int height, String buttonText, ResourceLocation background) {
	...
	public List<String> getToolTip(boolean extended) {
		...
		if (tooltipExtended.size() > 0) {
			if (extended) {button_tooltip.addAll(this.tooltipExtended);}
			else {button_tooltip.add(TextFormatting.GRAY + "" + TextFormatting.ITALIC + "Shift for more...");}
		}
